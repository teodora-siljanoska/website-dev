  ...
  useEffect(() => {
    ...
    domains.map((domain) => {
      void fetch('/api/domainRegistrationAttributes', {
        ...
        .then((response) => {
          response;
          let arrayOfQuestion: string[] = [];

          if (response.Attribute === undefined) {
            arrayOfQuestion = [];
          } else {
            if (response.Attribute.length > 0) {
              response.Attribute.map((attr: any) =>
                arrayOfQuestion.push(attr.Name)
              );
            } else {
              arrayOfQuestion.push(response.Attribute.Name);
            }
          }
